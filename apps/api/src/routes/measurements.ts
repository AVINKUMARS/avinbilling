...
measurementRouter.get('/', async (request, response, next) => {
  try {
    const filter: Record<string, unknown> = { ...tenantFilter(request.auth!), };
...
measurementRouter.post('/', requirePermission('measurements.create'), async (request, response, next) => {
  try {
    ...
    const count = await MeasurementItem.countDocuments({ ...tenantFilter(request.auth!),  projectId: input.projectId });
...
measurementRouter.delete('/:id', requirePermission('measurements.create'), async (request, response, next) => {
  try {
    const data = await MeasurementItem.findOneAndDelete({ _id: request.params.id, ...tenantFilter(request.auth!), status: { $ne: 'locked' } });
