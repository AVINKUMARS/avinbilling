...
export function BranchAdministration() {
  ...
  async function createWarehouse(event: FormEvent<HTMLFormElement>) {
    ...
    event.currentTarget.reset(); setMessage('Warehouse created.'); await load();
  }
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <form onSubmit={(event) => void createWarehouse(event).catch((error) => setMessage(error.message))} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card">
      ...
      <form onSubmit={(event) => void transferProject(event).catch((error) => setMessage(error.message))} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card">
