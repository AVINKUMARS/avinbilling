...
export type QuotePdfData = {
  ...
  documentTemplate?: { title: string; blocks: string[]; accentColor: string; footer?: string; showLogo: boolean };
};
...
function QuoteDocument({ quote }: { quote: QuotePdfData }) {
  ...
  return <Document><Page size="A4" style={styles.page}>
    ...
    <View style={styles.footer}><Text>{template?.footer ?? documents?.footerText ?? 'Generated by Avin Business Suite'}</Text><Text>{quote.quoteNumber}</Text></View>
  </Page></Document>;
}
