...
import { BarChart3, Building2, CircleDollarSign, Factory, FileText, Gauge, Layers3, LogOut, Menu, Package, Plus, ReceiptIndianRupee, Ruler, Search, Settings, ShoppingCart, Tags, Trash2, Truck, Users, WandSparkles, X } from 'lucide-react';
...
type RateCard = { _id: string; name: string; version: number; customerType: string; effectiveFrom: string; defaultWastagePercent: number; status: string; lines: Array<{ catalogItemId?: CatalogItem; purchaseRatePaise: number; sellingRatePaise: number; wastagePercent: number }> };
  ...
  return <main className="theme-app min-h-screen bg-canvas lg:grid lg:grid-cols-[260px_1fr]">
    ...
    <section className="min-w-0">
      ...
      <div className="p-5 lg:p-8">
        ...
        {page === 'rates' && <RateCardList cards={rateCards} onChanged={load} />}
    ...
    {modal === 'rate' && <Modal title="Create multi-item rate card" onClose={() => setModal(null)}><RateCardForm products={products} onSaved={async () => { setModal(null); await load(); }} /></Modal>}
...
type EditableRateLine = { catalogItemId: string; purchaseRate: string; sellingRate: string; wastagePercent: string };
function RateCardForm({ products, onSaved }: { products: CatalogItem[]; onSaved: () => Promise<void> }) {
  const blank = (): EditableRateLine => ({ catalogItemId: products[0]?._id ?? '', purchaseRate: '', sellingRate: '', wastagePercent: '0' });
  const [lines, setLines] = useState<EditableRateLine[]>([blank()]); const [message, setMessage] = useState(''); const [saving, setSaving] = useState(false);
  const updateLine = (index: number, key: keyof EditableRateLine, value: string) => setLines((current) => current.map((line, lineIndex) => lineIndex === index ? { ...line, [key]: value } : line));
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setSaving(true); setMessage(''); const data = new FormData(event.currentTarget);
    try {
      await apiRequest('/catalog/rate-cards', { method: 'POST', body: JSON.stringify({ name: data.get('name'), customerType: data.get('customerType'), effectiveFrom: data.get('effectiveFrom'), defaultWastagePercent: Number(data.get('defaultWastagePercent')), lines: lines.map((line) => ({ catalogItemId: line.catalogItemId, purchaseRatePaise: Math.round(Number(line.purchaseRate) * 100), sellingRatePaise: Math.round(Number(line.sellingRate) * 100), wastagePercent: Number(line.wastagePercent) })) }) });
      await onSaved();
    } catch (problem) { setMessage(problem instanceof Error ? problem.message : 'Unable to create rate card'); setSaving(false); }
  }
  return <form onSubmit={submit} className="mt-6 space-y-4"><div className="grid gap-4 sm:grid-cols-2"><Field label="Rate card name"><Input name="name" placeholder="UPVC Retail" required /></Field><Field label="Customer type"><select name="customerType" className="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3"><option value="retail">Retail</option><option value="dealer">Dealer</option><option value="builder">Builder</option><option value="project">Project</option><option value="custom">Custom</option></select></Field></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Effective from"><Input name="effectiveFrom" type="date" defaultValue={new Date().toISOString().slice(0, 10)} required /></Field><Field label="Default wastage %"><Input name="defaultWastagePercent" type="number" min="0" max="100" step="0.01" defaultValue="10" required /></Field></div><div className="border-t border-slate-200 pt-4"><div className="mb-3 flex items-center justify-between"><h3 className="font-black text-ink">Products and prices</h3><button type="button" onClick={() => setLines((current) => [...current, blank()])} className="rounded-lg border border-slate-300 px-3 py-2 text-xs font-bold"><Plus className="mr-1 inline" size={14} />Add item</button></div><div className="space-y-3">{lines.map((line, index) => <div key={index} className="rounded-xl border border-slate-200 bg-slate-50 p-3"><div className="flex gap-2"><select value={line.catalogItemId} onChange={(event) => updateLine(index, 'catalogItemId', event.target.value)} required className="min-w-0 flex-1 rounded-lg border border-slate-300 bg-white px-3 py-2">{products.map((item) => <option key={item._id} value={item._id}>{item.code} — {item.name} / {item.unit}</option>)}</select><button type="button" disabled={lines.length === 1} onClick={() => setLines((current) => current.filter((_, lineIndex) => lineIndex !== index))} className="grid size-10 place-items-center rounded-lg text-red-600 disabled:opacity-30"><Trash2 size={17} /></button></div><div className="mt-3 grid grid-cols-3 gap-2"><Input aria-label={`Purchase price ${index + 1}`} value={line.purchaseRate} onChange={(event) => updateLine(index, 'purchaseRate', event.target.value)} type="number" min="0" step="0.01" placeholder="Purchase ₹" required /><Input aria-label={`Selling price ${index + 1}`} value={line.sellingRate} onChange={(event) => updateLine(index, 'sellingRate', event.target.value)} type="number" min="0" step="0.01" placeholder="Selling ₹" required /><Input aria-label={`Wastage ${index + 1}`} value={line.wastagePercent} onChange={(event) => updateLine(index, 'wastagePercent', event.target.value)} type="number" min="0" max="100" step="0.01" placeholder="Waste %" required /></div></div>)}</div></div>{message && <div className="rounded-xl bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">{message}</div>}<button disabled={saving} className="flex w-full items-center justify-center gap-2 rounded-xl bg-brand-600 px-5 py-3.5 font-bold text-white disabled:opacity-50"><Plus size={17} />{saving ? 'Saving…' : `Save ${lines.length}-item rate card`}</button></form>;
}
function RateCardList({ cards, onChanged }: { cards: RateCard[]; onChanged: () => Promise<void> }) {
  const [message, setMessage] = useState(''); const money = (paise: number) => new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(paise / 100);
  async function activate(id: string) { try { await apiRequest(`/catalog/rate-cards/${id}/activate`, { method: 'POST' }); await onChanged(); } catch (problem) { setMessage(problem instanceof Error ? problem.message : 'Unable to activate'); } }
  async function remove(id: string) { if (!window.confirm('Delete this draft rate card?')) return; try { await apiRequest(`/catalog/rate-cards/${id}`, { method: 'DELETE' }); await onChanged(); } catch (problem) { setMessage(problem instanceof Error ? problem.message : 'Unable to delete'); } }
  if (!cards.length) return <Empty icon={CircleDollarSign} title="No rate cards yet" text="Add products first, then create a rate card containing all required materials and prices." />;
  return <div className="mt-7 space-y-4">{message && <div className="rounded-xl bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">{message}</div>}{cards.map((card) => <article key={card._id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-2"><h2 className="text-lg font-black text-ink">{card.name}</h2><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${card.status === 'active' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{card.status}</span></div><p className="mt-1 text-sm text-slate-500">Version {card.version} · {card.customerType} · effective {new Date(card.effectiveFrom).toLocaleDateString('en-IN')}</p></div><div className="flex gap-2">{card.status === 'draft' && <><button onClick={() => void activate(card._id)} className="rounded-lg bg-brand-600 px-3 py-2 text-xs font-bold text-white">Activate</button><button onClick={() => void remove(card._id)} className="rounded-lg border border-red-200 px-3 py-2 text-xs font-bold text-red-600">Delete</button></>}</div></div><div className="mt-4 overflow-x-auto"><table className="w-full text-left text-sm"><thead className="text-xs uppercase text-slate-400"><tr><th className="py-2">Item</th><th>Purchase</th><th>Selling</th><th>Waste</th></tr></thead><tbody className="divide-y divide-slate-100">{card.lines.map((line, index) => <tr key={index}><td className="py-3 font-bold text-ink">{line.catalogItemId?.code} — {line.catalogItemId?.name}</td><td>{money(line.purchaseRatePaise)}</td><td>{money(line.sellingRatePaise)}</td><td>{line.wastagePercent ?? 0}%</td></tr>)}</tbody></table></div></article>)}</div>;
}
